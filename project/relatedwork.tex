...
% Kusiak et al., 2009
\noindent One of the most complete studies of the wind farm layout optimization problem was done by Kusiak et al. (2009). \textcolor{red}{Should I write et al. when only two authors?} The study is based on six assumptions, which according to the authors are realistic and industrial-accepted. The study assume a fixed, predetermined turbine count, small variations of surface roughness, turbines with equal power curves, wind speed is following the Weibull distribution, that wind speed at different locations with same direction share the same Weibull distribution, and last, it assumes that any two turbines are separated with at least four rotor diameters. A multi-objective function was used to calculate the fitness of the solutions, which consisted of one objective function to maximize expected energy produced, and one to minimize the constraint violations. \\

% Different from Mosetti and Grady (page 1, orange): Thier wind energy calculation is not based on the power curve function, and wind direction was not fully discussed in their optimization models.

% Assumptions: 6 of them. Maybe this text should be a page or two, this model is important. Rewrite them! Did not understand them when i wrote them.
